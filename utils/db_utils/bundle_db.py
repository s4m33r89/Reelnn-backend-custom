...
class BundleDatabase:
    def __init__(self):
        ...
        # NEW: Index for the short hash used in Telegram links
    ...
    def get_bundles_for_show(self, show_id: Optional[Union[int, str]]) -> List[Dict[str, Any]]:
        """
        Get all bundles for a specific show_id (handles int/str mismatch).
        """
        try:
            if show_id is None:
                return []
            
            # Try finding by integer ID first (Standard)
            try:
            ...
            # If no results, try string ID (Legacy/Fallback)
            if not bundles:
    ...
    def get_bundle_by_hash_or_id(self, identifier: str) -> Optional[Dict[str, Any]]:
        """
        Find a bundle by its file_hash (preferred) or file_id (fallback).
        """
        # 1. Try Hash (Short link)
        bundle = self.collection.find_one({"file_hash": identifier})
        if bundle:
            return bundle
            
        # 2. Try File ID (Legacy/Internal)
